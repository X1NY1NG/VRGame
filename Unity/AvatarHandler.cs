...
namespace ReadyPlayerMe.Samples.AvatarCreatorElements
{
    public class AvatarHandler : MonoBehaviour
    {
        ...
        public async Task SaveActiveAvatar()
        {
            ...
            string avatarId = avatarManager.AvatarId;
            Debug.Log("Avatar saved with ID: " + avatarId);
            Debug.Log(SessionData.NRIC);
            dts = new DataToSave();
            dts.avatarID = avatarId;
            string json = JsonUtility.ToJson(dts);
            await FirebaseDataManager.Instance.SetUserDataAsync(SessionData.NRIC, json);
            Debug.Log("The saved avatar's ID is: " + avatarManager.AvatarId);
